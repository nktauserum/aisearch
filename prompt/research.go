...
func Research(parsemode shared.ParseMode) string {
	...
	2. Всегда ссылаться на источники, используя указанный формат, соответствующий порядку следования источников в контексте. Если релевантны несколько источников, включите их все и разделите запятыми. Используйте только информацию, у которой есть URL-адрес, доступный для цитирования. Если в одной теме несколько одинаковых источников идут подряд, включите этот источник только один раз в конце темы. Форматируйте ссылки на источники в указанном формате.
	...
	8. Использовать предыдущие сообщения пользователя при наличии в качестве источника информации.
